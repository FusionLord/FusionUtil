...
public class WavefrontObject implements IModelCustom
{
	...
	private void loadObjModel(InputStream inputStream) throws ModelFormatException
	{
		...
		{
			throw new net.fusionlord.fusionutil.client.objLoader.ModelFormatException("IO Exception reading model format", e);
		}
		finally
		{
			try
	...
	{
		groupObjects.stream().filter(groupObject -> partName.equalsIgnoreCase(groupObject.name)).forEach(GroupObject::render);
	}

	@SideOnly(Side.CLIENT)
	public void tessellatePart(WorldRenderer worldRenderer, String partName)
	{
		groupObjects.stream().filter(groupObject -> partName.equalsIgnoreCase(groupObject.name)).forEach(groupObject -> groupObject.render(worldRenderer));
	}
